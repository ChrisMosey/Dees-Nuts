...
Shader "Ciconia Studio/Effects/Triplanar/Diffuse Bump (1map)" {
    Properties {
        ...
        [Space(45)][Header(Main Maps)]
        [Space(10)]_MainTex ("Diffuse Top/Down (Y)", 2D) = "white" {}
        [Space(25)]_TilingY ("Tiling Y", Float ) = 1
        _RotationY ("Rotation Y", Float ) = 0
        [Space(25)]_TilingX ("Tiling X", Float ) = 1
        _RotationX ("Rotation X ", Float ) = 0
        [Space(25)]_TilingZ ("Tiling Z", Float ) = 1
        _RotationZ ("Rotation Z", Float ) = 0

        [Space(15)]_NormalTopDownY ("Normal Top/Down (Y)", 2D) = "bump" {}
        [Space(10)]_NormalIntensityY ("Normal Intensity Y", Range(0.01, 2)) = 1
        _NormalIntensityX ("Normal Intensity X", Range(0.01, 2)) = 1
        _NormalIntensityZ ("Normal Intensity Z", Range(0.01, 2)) = 1
    }
    SubShader {
        Tags {
            "RenderType"="Opaque"
        ...
        Pass {
            ...
            uniform float _NormalIntensityY;
            uniform sampler2D _NormalTopDownY; uniform float4 _NormalTopDownY_ST;
            uniform float _NormalIntensityX;
            uniform float _NormalIntensityZ;
            struct VertexInput {
            ...
            float4 frag(VertexOutput i) : COLOR {
                ...
                float3 node_4335 = NormalDirectionMask;
                ...
                float2 GB = node_9302;
                float2 node_5066 = GB;
                float3 _N1 = UnpackNormal(tex2D(_NormalTopDownY,TRANSFORM_TEX(node_5066, _NormalTopDownY))); // X ForntBack
                ...
                float2 RB = node_4319;
                float2 node_637 = RB;
                float3 _N2 = UnpackNormal(tex2D(_NormalTopDownY,TRANSFORM_TEX(node_637, _NormalTopDownY))); // Y TopDown
                ...
                float2 RG = node_8575;
                float2 node_1837 = RG;
                float3 _node_6848 = UnpackNormal(tex2D(_NormalTopDownY,TRANSFORM_TEX(node_1837, _NormalTopDownY))); // Z LeftRight
                float3 normalLocal = normalize(((node_4335.r*(lerp(float3(0,0,1),_N1.rgb,_NormalIntensityX)*NormalDirectionMask.r) + node_4335.g*(lerp(float3(0,0,1),_N2.rgb,_NormalIntensityY)*NormalDirectionMask.r) + node_4335.b*(lerp(float3(0,0,1),_node_6848.rgb,_NormalIntensityZ)*NormalDirectionMask.r))*lerp(0,2,_NormalIntensityY)));
...
/////// Diffuse:
                ...
                float2 node_9874 = GB;
                float4 _node_1257 = tex2D(_MainTex,TRANSFORM_TEX(node_9874, _MainTex)); // X Axis FrontBack
                float2 node_7446 = RB;
                float4 _node_4330 = tex2D(_MainTex,TRANSFORM_TEX(node_7446, _MainTex)); // Y Axis TopBottom
                float2 node_4282 = RG;
                float4 _node_3650 = tex2D(_MainTex,TRANSFORM_TEX(node_4282, _MainTex)); // Z Axis LeftRight 
