...
Shader "Ciconia Studio/Effects/Triplanar/Diffuse Bump (3map)" {
    Properties {
        ...
        _NormalIntensityY ("Normal Intensity Y", Range(0.01, 2)) = 1
        ...
        [Space(15)]_NormalSideX ("Normal Side (X)", 2D) = "bump" {}
        ...
        [Space(15)]_NormalSideZ ("Normal Side (Z)", 2D) = "bump" {}
    ...
    SubShader {
        ...
        Pass {
            ...
            uniform sampler2D _NormalSideX; uniform float4 _NormalSideX_ST;
            uniform sampler2D _DiffuseSideZ; uniform float4 _DiffuseSideZ_ST;
            uniform sampler2D _NormalSideZ; uniform float4 _NormalSideZ_ST;
            struct VertexInput {
            ...
            float4 frag(VertexOutput i) : COLOR {
                ...
                float2 GB = node_9302;
                float2 node_5066 = GB;
                float3 _N1 = UnpackNormal(tex2D(_NormalSideX,TRANSFORM_TEX(node_5066, _NormalSideX))); // X ForntBack
                ...
                float2 RG = node_8575;
                float2 node_1837 = RG;
                float3 _node_6848 = UnpackNormal(tex2D(_NormalSideZ,TRANSFORM_TEX(node_1837, _NormalSideZ))); // Z LeftRight
